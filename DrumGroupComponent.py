...
from Lewp import liveobj_valid

from PadControl import PadControl   # added 10/17
...
class DrumGroupComponent(SlideComponent, Slideable):
    ...
    def _set_position(self, index):
        if not 0 <= index <= 28:
            ...
            self._drum_group_device.view.drum_pads_scroll_position = self._drum_group_device and index   # this was moved back btw 22/10/17 sun PM was yellow lined
      #  self._drum_group_device.view.drum_pads_scroll_position = liveobj_valid(self._drum_group_device) and index  # taken from V2
    ...
    def _button_coordinates_to_pad_index(self, first_note, coordinates):    
        """
        Alters the hight and width of the drum matrix
        """
        y, x = coordinates
        y = self.height - y - 1
        if x < 8 and y >= 8:
            ...
            first_note += 8 * self.width
        elif x >= 8 and y >= 8:
            first_note += 8 * self.width + 64
        index = x % 8 + y % 8 * 8 + first_note
        return index
