...
from _Framework import Task  # added 10/17
...
from MatrixMaps import PLAYHEAD_FEEDBACK_CHANNELS  # added 10/17 for further down
...
class NoteEditorComponent(CompoundComponent, Subject):
    ...
    def __init__(self, settings_mode = None, clip_creator = None, grid_resolution = None, skin_base_key = 'NoteEditor', *a, **k):
        
        if not skin_base_key is not None: #or AssertionError
...
# added 10/17 so if need be switchback to below
  ...
  #      self._modify_task.kill()
        ...
        self.background_color = 'NoteEditor.StepEmpty'  #.StepEmpty
...
# the following greened out. 10/17. 
...
#        self.background_color = self._skin_base_key + '.StepEmpty' 
   ...
   #     self.background_color = 'NoteEditor.StepEmpty'   # NoteEditor.StepEmpty    # color of note matrix    # this was already out 10/17. not sure why
...
# continue
    ...
    def set_button_matrix(self, matrix):
        ...
        if matrix:
          ...
          #      button.set_channel(PAD_FEEDBACK_CHANNEL)   switched 10/17
